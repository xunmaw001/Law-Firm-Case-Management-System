<template>
    <div class="main-content">
        <el-dialog title="审核" :visible.sync="anyuanYesnoTypesVisible">
            <el-form :model="form">
                <input type="hidden" v-model="form.id">
                <el-form-item label="审核" >
                    <el-select v-model="form.anyuanYesnoTypes" placeholder="请选择收案审批">
                        <el-option label="同意收案" value="2"></el-option>
                        <el-option label="拒绝收案" value="3"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="审批回复">
                    <el-input type="textarea" v-model="form.anyuanYesnoText" placeholder="审批回复"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="anyuanYesnoTypesVisible = false">取 消</el-button>
                <el-button type="primary" @click="anyuanYesnoTypesShenhe">审 核</el-button>
            </div>
        </el-dialog>
        <!-- 条件查询 -->
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '收案审批' : ''">
                        <el-select v-model="searchForm.anyuanYesnoTypes" placeholder="请选择收案审批">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in anyuanYesnoTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '客户名称' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.yonghuName" placeholder="客户名称" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('anyuan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/lvshishiwusuoanjianguanli/upload/anyuanMuBan.xls"
                        >批量导入案源信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('anyuan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="anyuanUploadSuccess"
                                :on-error="anyuanUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('anyuan','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入案源信息数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('anyuan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "anyuan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="anyuanYesnoTypes"
                                      header-align="center"
                                      label="收案审批">
                        <template slot-scope="scope">
                            {{scope.row.anyuanYesnoValue}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="anyuanYesnoText"
                                      header-align="center"
                                      label="审批回复">
                        <template slot-scope="scope">
                            <span v-if="scope.row.anyuanYesnoText != null &&scope.row.anyuanYesnoText.length>10">
                                {{scope.row.anyuanYesnoText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.anyuanYesnoText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('anyuan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('anyuan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('anyuan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <el-button v-if="isAuth('anyuan','审核') && scope.row.anyuanYesnoTypes == 1 " type="primary" icon="el-icon-thumb" size="mini" @click="openYesnoTypes(scope.row.id)">审核</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                anyuanUuidNumber : null,
                anyuanName : null,
                anyuanTypes : null,
                yonghuId : null,
                anyuanContent : null,
                anyuanYesnoTypes : null,
                anyuanYesnoText : null,
                anyuanDelete : null,
                insertTime : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'anyuan',//当前表表名,
                        sumColum : 'anyuan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'anyuan_name',//分组字符串字段
                        // types : 'anyuan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['anyuanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            anyuanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "anyuan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            openYesnoTypes(id) {
                ...
                _this.form.id = null;
                _this.form.id = id;
                            _this.form.anyuanYesnoText = null;
                _this.form.anyuanYesnoTypes = "请选择收案审批";
                _this.anyuanYesnoTypesVisible = true;
            },

            anyuanYesnoTypesShenhe() {
                let _this = this;
                if(_this.form.anyuanYesnoTypes == "请选择收案审批"){
                    _this.$message.error("请选择收案审批");
                    return false;
                }
                if(_this.form.anyuanYesnoText == null || _this.form.anyuanYesnoText == ""){
                ...
                this.$http({
                    ...
                    if (data && data.code === 0) {
                        this.$message({
                            ...
                            duration: 1500,
                            onClose: () => {
                                _this.anyuanYesnoTypesVisible = false;
                                _this.search();
                            }
                        });
                    } else {
